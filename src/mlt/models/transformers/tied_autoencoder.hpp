...
namespace transformers {
	...
	class TiedAutoencoder : public BaseModel, public TransformerMixin<TiedAutoencoder<HiddenActivation, ReconstructionActivation, Optimizer>> {
	public:
		...
		TiedAutoencoder& fit(const Eigen::Ref<const Eigen::MatrixXd>& input, bool cold_start = true) {
		...
		using TransformerMixin<TiedAutoencoder<HiddenActivation, ReconstructionActivation, Optimizer>>::fit;
		...
		auto loss(const Eigen::Ref<const Eigen::VectorXd>& coeffs, const Eigen::Ref<const Eigen::MatrixXd>& input, const Eigen::Ref<const Eigen::MatrixXd>& target) const {
			...
			return implementations::autoencoder::loss(_hidden_activation, _reconstruction_activation, weights, hidden_intercepts,
				weights.transpose(), reconstruction_intercepts, this->_regularization, input, target);
		}
		...
		auto gradient(const Eigen::Ref<const Eigen::VectorXd>& coeffs, const Eigen::Ref<const Eigen::MatrixXd>& input, const Eigen::Ref<const Eigen::MatrixXd>& target) const {
			...
			std::tie(weights_grad, hid_inter_grad, weights_transp_grad, rec_inter_grad) = implementations::autoencoder::gradient(_hidden_activation,
		...
		std::tuple<double, Eigen::VectorXd> loss_and_gradient(const Eigen::Ref<const Eigen::VectorXd>& coeffs, const Eigen::Ref<const Eigen::MatrixXd>& input, const Eigen::Ref<const Eigen::MatrixXd>& target) const {
			...
			Eigen::MatrixXd weights_grad, weights_transp_grad;
			Eigen::VectorXd hid_inter_grad, rec_inter_grad;
			std::tie(loss, weights_grad, hid_inter_grad, weights_transp_grad, rec_inter_grad) = implementations::autoencoder::loss_and_gradient(
				_hidden_activation, _reconstruction_activation, weights, hidden_intercepts, weights.transpose(), reconstruction_intercepts,
				this->_regularization, input, target);
